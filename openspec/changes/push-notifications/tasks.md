...
- [x] 9.1 Backend unit: resolver de destinatarios (empleado / fan-out admins en MANUAL / no-admin en AUTOMATIC); regla de entrega efectiva `global ∧ empleado` por canal (incl. empleado silenciado); aviso al empleado en cancelación admin y no-autoaviso en cancelación propia; borrado ante 410.
- [x] 9.2 Backend web (`@WebMvcTest`): RBAC de los endpoints (401/403), alta idempotente, baja, cambio de flags globales solo ADMIN, flags por empleado en el formulario solo ADMIN.
...
- [x] 10.1 Backend (JDK 21): `mvn clean verify` verde; ArchUnit/Sonar sin violations nuevas.

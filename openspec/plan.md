...
| # | Change | Tipo | Estado | % |
|---|--------|------|--------|---|
| C0 | `design-system-components` | Frontend (UI) + Backend | 🔄 **PR #82 abierta, CI verde** (Issue #81, rama `feat/frontend/81-design-system-base`). WP0 fundación ✅ · WP-BACK asignación fija (DELETE/GET por resourceType, 177 tests) ✅ · WP1 empleados+modal pestañas plaza/puesto (elimina FixedAssignmentsPage) ✅ · WP2 auth/preferencias (popover, tema switch, sesión ámbar) ✅ · WP3 solicitudes/calendario (tabs caja+count, info-banners, footer bar) ✅ · WP4 plazas/puestos/visitantes/auditoría (toggle real, legend, pills por color) ✅ · WP5 móvil/plano (week-cards, editor, release resumen) ✅. Frontend 402 tests + Backend 177 tests verdes; **CI Backend+Frontend PASS**. Login verificado en navegador (paridad OK). Pendiente: verificación ocular de pantallas autenticadas (stack local bloqueado por SQL Server/JDBC del host — no es del código) + merge tras CodeRabbit. Regla aplicada: donde el mockup solo dibuja plaza, también puesto. | `█████████░` 90% |

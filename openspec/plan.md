...
| # | Change | Tipo | Estado | % |
|---|--------|------|--------|---|
| C0 | `design-system-components` | Frontend (UI) + Backend | 🔄 **En curso** (Issue #81, rama `feat/frontend/81-design-system-base`). Auditoría 5/5 completada: ~14 clases del design-system ausentes/incompletas en `components.css` → causa raíz de las divergencias. Plan aprobado: WP0 fundación (clases+componentes reutilizables+Modal con tabs) · WP-BACK asignación fija robusta (DELETE/GET por resourceType) · WP1 empleados+asignación fija en modal (elimina FixedAssignmentsPage) · WP2 auth/preferencias (popover, tema toggle, sesión ámbar) · WP3 solicitudes/calendario (tabs caja+count, info-banners, footer bar) · WP4 plazas/puestos/visitantes/auditoría (toggle real, legend, pills por color) · WP5 móvil/plano (week-cards, editor, release resumen). Regla: donde el mockup solo dibuja plaza, implementar también puesto. | `░░░░░░░░░░` 5% |

...
**Regla de dependencias:** todo apunta hacia el dominio. Los adaptadores conocen los puertos; el dominio no conoce a los adaptadores (inversión de dependencias). Frente al esquema clásico de capas (controller → service → repository), en los módulos hexagonales el `service` pasa a ser el **núcleo de dominio** detrás de puertos, y el `repository` concreto es **un adaptador más**, intercambiable.

> **Estado real de la persistencia (hexagonalidad por complejidad, change #89).** La hexagonalidad de persistencia se aplica de forma **proporcional al dominio**, no uniforme:
> - **Agregados de dominio rico con invariantes de concurrencia** — `request`, `release`, `fixedassignment` — están **plenamente hexagonalizados**: modelo de dominio libre de JPA en `…/domain`, **puerto de repositorio** (`…RepositoryPort`) + **adaptador** (`…PersistenceAdapter`) sobre Spring Data en `…/infrastructure`, y **mapper** entidad↔dominio. Una regla **ArchUnit** (`HexagonalArchitectureTest`) verifica que su dominio no depende de JPA/Hibernate/Spring Data-repository y que su aplicación no depende de `JpaRepository` ni de la entidad. (Se permite `org.springframework.data.domain.Pageable`/`Page` como value type en la frontera.)
> - **Módulos CRUD** — `desk`, `parkingspace`, `employee`, `visitor`, `auditlog`, `loginlog`, `emailoutbox` — siguen el modelo **Spring pragmático** (la entidad JPA es el modelo y el `service` usa `JpaRepository` directo). Es una decisión deliberada: introducir puertos de repositorio en un CRUD es sobre-ingeniería sin retorno.
> - Los **puertos de infraestructura transversal** (`ClockPort`, `EmailSenderPort`, `ExportWriterPort`, `ResourceResolverPort`, `ReleaseAuditPort`, `RetentionPurgePort`, `LoginLogRecorder`) sí existen de forma general, con sus adaptadores.

export const es = {
  common: {
    appName: 'Reservas',
    brandSub: 'Nexo',
    sortBy: 'Ordenar por {{column}}',
    logout: 'Cerrar sesión',
    language: 'Idioma',
    theme: 'Tema',
    themeLight: 'Claro',
    themeDark: 'Oscuro',
    close: 'Cerrar',
    cancel: 'Cancelar',
    confirm: 'Confirmar',
    save: 'Guardar',
    loading: 'Cargando…',
    retry: 'Reintentar',
    search: 'Buscar',
    filters: 'Filtros',
    listAnd: 'y',
    dayIncluded: 'incluido',
    dayExcluded: 'no incluido',
    weekdayChip: { 1: 'L', 2: 'M', 3: 'X', 4: 'J', 5: 'V', 6: 'S', 7: 'D' },
    weekdayAbbr: {
      1: 'LUN',
      2: 'MAR',
      3: 'MIÉ',
      4: 'JUE',
      5: 'VIE',
      6: 'SÁB',
      7: 'DOM',
    },
    weekdayName: {
      1: 'Lunes',
      2: 'Martes',
      3: 'Miércoles',
      4: 'Jueves',
      5: 'Viernes',
      6: 'Sábado',
      7: 'Domingo',
    },
  },
  auth: {
    loginTitle: 'Iniciar sesión',
    loginSubtitle: 'Gestión de parking',
    loginDescription: 'Acceso con tu usuario corporativo.',
    brandTitleLead: 'Gestión de',
    brandTitleHighlight: 'plazas y puestos',
    brandSubtitle: 'Reserva tu plaza de parking o puesto de trabajo en segundos.',
    secureFooter: 'Acceso seguro',
    loginField: 'Usuario',
    passwordField: 'Contraseña',
    signIn: 'Entrar',
    invalidCredentials: 'Usuario o contraseña incorrectos',
    invalidCredentialsAttempts:
      'Usuario o contraseña incorrectos. Te quedan {{count}} intentos antes del bloqueo temporal.',
    showPassword: 'Mostrar contraseña',
    hidePassword: 'Ocultar contraseña',
    forgotPassword: '¿Has olvidado tu contraseña? Solicita un restablecimiento al administrador.',
    lockoutNote: 'Tras 5 intentos fallidos la cuenta se bloquea 15 minutos.',
    sessionExpiredTitle: 'Sesión expirada',
    sessionExpiredBody: 'Tu sesión ha caducado. Vuelve a iniciar sesión.',
    sessionExpiredHeading: 'Tu sesión ha caducado',
    sessionExpiredDetail:
      'Por seguridad, la sesión se ha cerrado tras un periodo de inactividad (o ha sido invalidada). Vuelve a iniciar sesión para continuar.',
    sessionExpiredPhaseNote:
      'En Fase 2 se redirige a la landing de ALEATICA para volver a autenticarse. En Fase 1 vuelve a la pantalla de login.',
    backToLogin: 'Volver a iniciar sesión',
    changePasswordTitle: 'Cambiar contraseña',
    changePasswordIntro:
      'Debes establecer una nueva contraseña antes de continuar (primer acceso tras un restablecimiento).',
    currentPassword: 'Contraseña actual',
    newPassword: 'Nueva contraseña',
    confirmPassword: 'Confirmar nueva contraseña',
    changePasswordCta: 'Cambiar contraseña',
    passwordMismatch: 'Las contraseñas no coinciden',
    changePasswordSuccess: 'Contraseña actualizada',
    showcaseTagline: 'Encuentra tu plaza o tu puesto en segundos, sin fricción.',
    showcasePoint1: 'Disponibilidad en tiempo real de plazas y puestos',
    showcasePoint2: 'Solicita, libera y consulta tu semana desde el móvil',
    showcasePoint3: 'Trazabilidad clara: quién pidió, quién aprobó',
    policy: {
      length: 'Al menos 10 caracteres',
      upper: 'Una letra mayúscula',
      lower: 'Una letra minúscula',
      digit: 'Un dígito',
      symbol: 'Un símbolo',
    },
  },
  account: {
    openMenu: 'Abrir menú de usuario',
    menuLabel: 'Menú de usuario',
    logoutConfirm: {
      title: 'Cerrar sesión',
      body: '¿Seguro que deseas cerrar sesión?',
      confirm: 'Cerrar sesión',
      cancel: 'Cancelar',
    },
    role: {
      ADMIN: 'Administrador',
      EMPLOYEE: 'Empleado',
      AGENCIA: 'Agencia',
    },
  },
  layout: {
    brandTagline: 'Gestión de parking',
    adminArea: 'Administración',
    employeeArea: 'Mi espacio',
    agencyArea: 'Agencia',
    sections: {
      management: 'Gestión',
      operations: 'Operativa',
      navigation: 'Navegación',
    },
    nav: {
      open: 'Abrir menú de navegación',
      close: 'Cerrar menú de navegación',
      menuLabel: 'Navegación principal',
      newReservation: 'Nueva reserva',
    },
  },
  notifications: {
    onboarding: {
      activateTitle: 'Activa las notificaciones',
      activateBody: 'Recibe al instante cuando te confirmen, rechacen o cambien una reserva.',
      activateAction: 'Activar notificaciones',
      installTitle: 'Instala la app',
      installBody: 'Instala la app para acceder más rápido y recibir notificaciones.',
      installAction: 'Instalar',
      iosTitle: 'Instala la app para recibir avisos',
      iosBody:
        'En iPhone/iPad, pulsa el botón Compartir y elige "Añadir a pantalla de inicio". Luego abre la app y activa las notificaciones.',
      deniedTitle: 'Notificaciones bloqueadas',
      deniedBody:
        'Has bloqueado las notificaciones. Actívalas desde los ajustes del navegador para este sitio.',
    },
  },
  dashboard: {
    navLabel: 'Panel',
    eyebrow: 'Panel',
    greeting: {
      morning: 'Buenos días',
      afternoon: 'Buenas tardes',
      evening: 'Buenas noches',
    },
    seeWeek: 'Ver semana',
    kpi: {
      occupancy: 'Ocupación de hoy',
      occupancySub: '{{occupied}} de {{total}} recursos ocupados',
      pending: 'Solicitudes pendientes',
      pendingSub_one: '{{count}} nueva hoy',
      pendingSub_other: '{{count}} nuevas hoy',
      parkingFree: 'Plazas libres',
      parkingFreeSub: 'de {{total}} plazas de parking',
      deskFree: 'Puestos libres',
      deskFreeSub: 'de {{total}} puestos de oficina',
    },
    pending: {
      title: 'Solicitudes pendientes',
      seeAll: 'Ver todas →',
      empty: 'No hay solicitudes pendientes. Todo al día.',
      approve: 'Aprobar',
      pill: 'Pendiente',
      resourceLine: '{{resource}} · {{date}}',
    },
    activity: {
      title: 'Actividad reciente',
      empty: 'Sin actividad reciente.',
      system: 'Sistema',
      actor: 'por {{actor}}',
      action: {
        VOLUNTARY_RELEASE: 'Liberación voluntaria',
        ADMINISTRATIVE_RELEASE: 'Liberación administrativa',
        CANCEL_RELEASE: 'Liberación deshecha',
        CANCEL_APPROVED_REQUEST: 'Recurso liberado',
        ADMIN_CANCEL_APPROVED_REQUEST: 'Liberación por el administrador',
        ADMIN_PUNCTUAL_ASSIGNMENT: 'Asignación puntual',
        ADMIN_REASSIGN_RESOURCE: 'Reasignación de recurso',
        ADMIN_SWAP_RESOURCES: 'Intercambio de recursos',
        APPROVE_REQUEST: 'Solicitud aprobada',
        REJECT_REQUEST: 'Solicitud rechazada',
        CREATE_REQUEST: 'Solicitud creada',
        EXPORT_REQUESTS: 'Exportación de solicitudes',
        EXPORT_MY_REQUESTS: 'Exportación de solicitudes',
      },
      entity: {
        RELEASE: 'Liberación',
        REQUEST: 'Solicitud',
        EMPLOYEE: 'Empleado',
        VISITOR: 'Visitante',
        VISITORRESERVATION: 'Reserva de visitante',
        SYSTEMSETTINGS: 'Configuración',
        AUDITLOG: 'Auditoría',
      },
    },
    resourceType: {
      PARKING: 'Plaza',
      DESK: 'Puesto',
    },
  },
  resources: {
    navLabel: 'Recursos',
    title: 'Recursos',
    eyebrow: 'Gestión',
    description: 'Gestiona el inventario de plazas de parking y puestos de oficina.',
    tabs: {
      parking: 'Plazas de parking',
      desks: 'Puestos de trabajo',
    },
  },
  records: {
    navLabel: 'Registros',
    title: 'Registros',
    eyebrow: 'Gestión',
    description: 'Consulta la auditoría de acciones y el registro de accesos al sistema.',
    tabs: {
      audit: 'Auditoría',
      loginLogs: 'Accesos',
    },
  },
  occupancy: {
    navLabel: 'Ocupación',
    title: 'Ocupación',
    eyebrow: 'Operativa',
    tabs: {
      weekly: 'Semanal',
      availability: 'Disponibilidad',
    },
    weekly: {
      actionableHint: 'Pulsa una celda libre para asignar, o una ocupada para liberar.',
      resourceTypeLabel: 'Tipo de recurso',
      modeSwitchLabel: 'Cambiar entre plazas y puestos',
      modeTitle: {
        PARKING: 'Plazas de parking',
        DESK: 'Puestos de oficina',
      },
      resourceType: {
        PARKING: 'Plazas de parking',
        DESK: 'Puestos de trabajo',
      },
      resourceColumn: {
        PARKING: 'Plaza',
        DESK: 'Puesto',
      },
      summarySpaces: {
        PARKING: 'Plazas activas',
        DESK: 'Puestos activos',
      },
      kpi: {
        today: 'hoy',
        totalLabel: {
          PARKING: 'Plazas totales',
          DESK: 'Puestos totales',
        },
        occupied: 'Ocupados {{day}}',
        free: 'Libres {{day}}',
        released: 'Liberados {{day}}',
        inInventory: 'en inventario',
        ofOccupancy: '{{pct}}% de ocupación',
        availableNow: 'disponibles ahora',
        releasedSub: 'liberados temporalmente',
      },
      cellAction: {
        ASSIGN: 'Asignar {{resource}}',
        RELEASE_FIXED: 'Liberar {{resource}}',
        CANCEL_REQUEST: 'Liberar {{resource}}',
      },
      filterLabel: 'Filtrar por estado',
      filters: {
        all: 'Todos',
        free: 'Solo libres',
        occupied: 'Ocupados',
        released: 'Liberados',
        requests: 'Solicitudes',
      },
      noMatches: 'Ningún recurso coincide con el filtro.',
      viewInPlan: 'Ver el puesto {{resource}} en el plano',
      viewInPlanShort: 'Ver en plano',
      viewInPlanTip: 'Puesto {{resource}} en el plano',
      viewInPlanModalTitle: 'Puesto {{resource}} · en el plano',
    },
    assign: {
      title: 'Asignar recurso',
      summary: 'Resumen de la asignación',
      resourceType: {
        PARKING: 'Plaza',
        DESK: 'Puesto',
      },
      date: 'Fecha',
      beneficiary: {
        label: '¿Para quién?',
        employee: 'Empleado',
        visitor: 'Visitante',
      },
      employee: 'Empleado',
      selectEmployee: 'Selecciona un empleado',
      visitor: 'Visitante',
      selectVisitor: 'Selecciona un visitante',
      visitorHint: 'Se reserva el recurso para el visitante en esta fecha.',
      requiredVisitor: 'Selecciona un visitante.',
      modeLabel: 'Tipo de asignación',
      mode: {
        punctual: 'Solo esta fecha',
        fixed: 'Fija (cada semana)',
      },
      punctualHint: 'Se asigna el recurso al empleado únicamente para esta fecha.',
      fixedHint: 'Se asigna el recurso al empleado todos los {{weekday}}, de forma indefinida.',
      requiredEmployee: 'Selecciona un empleado.',
      submit: 'Asignar',
      cancel: 'Cancelar',
      done: 'Recurso asignado.',
      errors: {
        conflict: 'El recurso ya está ocupado esa fecha.',
        noAvailability: 'No hay recursos disponibles para esa fecha.',
        badRequest: 'Revisa los datos de la asignación.',
        generic: 'No se pudo completar la asignación.',
      },
    },
    manage: {
      title: 'Gestionar reserva',
      summary: 'Reserva actual',
      employee: 'Empleado',
      resourceType: {
        PARKING: 'Plaza',
        DESK: 'Puesto',
      },
      date: 'Fecha',
      actionLabel: '¿Qué quieres hacer?',
      action: {
        reassign: 'Reasignar',
        swap: 'Intercambiar',
        cancel: 'Liberar',
      },
      reassign: {
        select: 'Nuevo recurso',
        placeholder: 'Selecciona un recurso libre',
        empty: 'No hay recursos libres esa fecha.',
        hint: 'Se mueve la reserva del empleado al recurso elegido para esta fecha.',
        required: 'Selecciona un recurso.',
        submit: 'Reasignar',
        done: 'Reserva reasignada.',
      },
      swap: {
        select: 'Intercambiar con',
        placeholder: 'Selecciona otra reserva',
        empty: 'No hay otras reservas esa fecha para intercambiar.',
        hint: 'Ambos empleados intercambian su recurso en esta fecha.',
        required: 'Selecciona una reserva.',
        submit: 'Intercambiar',
        done: 'Recursos intercambiados.',
        option: '{{resource}} · {{employee}}',
      },
      cancel: {
        hint: 'Se cancela la reserva y el recurso queda libre para esta fecha.',
        submit: 'Liberar reserva',
      },
      close: 'Cerrar',
      errors: {
        conflict: 'El recurso ya no está libre esa fecha.',
        badRequest: 'No se pudo completar la operación con esos datos.',
        generic: 'No se pudo completar la operación.',
      },
    },
  },
  myResources: {
    navLabel: 'Mis sitios fijos',
    title: 'Mis sitios fijos',
    eyebrow: 'Mi espacio',
    description: 'Tus plazas y puestos fijos, y las liberaciones que has hecho.',
    tabs: {
      fixed: 'Sitios fijos',
      releases: 'Liberaciones',
    },
    released: {
      title: 'Días que has liberado',
      undo: 'Deshacer',
      undoAll: 'Deshacer todos',
      undoAllConfirmTitle: '¿Deshacer todas las liberaciones?',
      undoAllConfirmBody: 'Recuperarás tu sitio fijo en los {{count}} día(s) liberados. Otras personas dejarán de poder reservarlos.',
      undone: 'Liberación deshecha: recuperas tu sitio ese día.',
      undoFailed: 'No se pudo deshacer la liberación.',
    },
    absence: {
      ctaTitle: '¿Te vas de vacaciones o faltas algún día?',
      ctaSub: 'Libera tu plaza y tu puesto de los días que no vengas.',
      title: 'Liberar por ausencia',
      intro: 'Elige los días que no vas a venir; liberaremos tus recursos fijos de esos días para que otra persona pueda usarlos.',
      mode: {
        label: 'Cómo elegir los días',
        single: 'Días sueltos',
        range: 'Rango',
      },
      clear: 'Limpiar',
      addDay: 'Añadir un día',
      addRange: 'Añadir un rango',
      add: 'Añadir',
      from: 'Desde',
      to: 'Hasta',
      selectedDays: 'Días seleccionados',
      removeDay: 'Quitar día',
      whatToRelease: '¿Qué quieres liberar?',
      summary: 'Se liberarán {{count}} recurso(s): {{resources}}.',
      someAlreadyReleased: 'Ya habías liberado {{count}} de los días elegidos; esos se omiten.',
      allAlreadyReleased: 'Ya tenías liberados todos los días elegidos: no hay nada nuevo que liberar.',
      nothing: 'No tienes recursos fijos en los días elegidos: no hay nada que liberar.',
      confirm: 'Liberar ({{count}})',
      cancel: 'Cancelar',
      done: 'Recursos liberados.',
      failed: 'No se pudo liberar. Inténtalo de nuevo.',
    },
  },
  exports: {
    groupLabel: 'Opciones de exportación',
    export: 'Exportar',
    csv: 'Exportar CSV',
    xlsx: 'Exportar XLSX',
    myData: 'Exportar mis datos',
    errors: {
      rateLimit: 'Has alcanzado el límite de exportaciones. Inténtalo de nuevo en un minuto.',
      generic: 'No se pudo generar la exportación. Inténtalo de nuevo.',
    },
  },
  employees: {
    title: 'Empleados',
    eyebrow: 'Administración',
    description: 'Alta, edición y asignación fija de plazas y puestos por empleado.',
    navLabel: 'Empleados',
    searchPlaceholder: 'Buscar empleado…',
    searchLabel: 'Buscar empleado',
    new: 'Nuevo empleado',
    columns: {
      name: 'Empleado',
      email: 'Email',
      department: 'Departamento',
      parkingFixed: 'Plaza fija',
      deskFixed: 'Puesto fijo',
      role: 'Rol',
      category: 'Categoría',
      status: 'Estado',
      actions: 'Acciones',
    },
    note: 'Día con recurso fijo asignado · un empleado puede tener recursos distintos por día',
    legend: {
      assigned: 'Día con recurso fijo asignado (todo el año)',
      none: 'Sin recurso fijo (debe solicitar)',
    },
    role: {
      ADMIN: 'Administrador',
      EMPLOYEE: 'Empleado',
      AGENCIA: 'Agencia',
    },
    category: {
      CEO: 'CEO',
      CONSEJO: 'Consejo',
      DIRECTOR_N1: 'Director nivel 1',
      DIRECTOR_N2: 'Director nivel 2',
      GERENTE: 'Gerente',
      MANDO_INTERMEDIO: 'Mando intermedio',
      EMPLEADO: 'Empleado',
    },
    status: {
      active: 'Activo',
      inactive: 'De baja',
    },
    actions: {
      edit: 'Editar',
      deactivate: 'Dar de baja',
      reactivate: 'Reactivar',
      resetPassword: 'Restablecer contraseña',
      more: 'Más acciones',
    },
    deactivateConfirm: {
      title: '¿Dar de baja al empleado?',
      body: 'Se dará de baja a {{name}}. No podrá acceder ni tener asignaciones hasta reactivarlo.',
    },
    loading: 'Cargando empleados…',
    empty: 'No hay empleados que mostrar.',
    loadError: 'No se pudieron cargar los empleados.',
    pagination: {
      previous: 'Anterior',
      next: 'Siguiente',
      pageInfo: 'Página {{page}} de {{total}}',
    },
    vehicles: {
      tabLabel: 'Vehículos',
      saveFirst: 'Guarda primero el empleado para poder añadir vehículos.',
      empty: 'Este empleado no tiene vehículos.',
      loadError: 'No se pudieron cargar los vehículos.',
      add: 'Añadir vehículo',
      edit: 'Editar',
      editTitle: 'Editar vehículo',
      delete: 'Borrar',
      confirmDelete: 'Sí, borrar',
      deleteTitle: 'Borrar vehículo',
      deleteBody: '¿Seguro que quieres borrar el vehículo {{plate}}? Esta acción no se puede deshacer.',
      saveVehicle: 'Guardar vehículo',
      plateRequired: 'La matrícula es obligatoria.',
      plateTaken: 'Este empleado ya tiene un vehículo con esa matrícula.',
      saveError: 'No se pudo guardar el vehículo.',
      columns: {
        brand: 'Marca',
        model: 'Modelo',
        plate: 'Matrícula',
        color: 'Color',
        actions: 'Acciones',
      },
      form: {
        plate: 'Matrícula *',
        brand: 'Marca',
        model: 'Modelo',
        color: 'Color',
      },
    },
    form: {
      createTitle: 'Nuevo empleado',
      editTitle: 'Editar empleado',
      tabs: {
        details: 'DETALLES',
        parking: 'PLAZA FIJA',
        desk: 'PUESTO FIJO',
        vehicles: 'VEHÍCULOS',
        history: 'HISTÓRICO',
      },
      firstName: 'Nombre',
      lastName: 'Apellidos',
      login: 'Usuario / login',
      email: 'Email',
      department: 'Departamento',
      mobilePhone: 'Teléfono móvil',
      role: 'Rol',
      category: 'Categoría',
      isCorporate: 'Empleado corporativo',
      corporateOn: 'Sí (tiene EntraID)',
      corporateOff: 'No (usuario local)',
      notifications: 'Notificaciones',
      notifyEmail: 'Recibir avisos por email',
      notifyPush: 'Recibir avisos push',
      validity: 'Vigencia',
      validityIndefinite: 'Indefinida',
      resourceIncomplete: 'Selecciona el recurso y al menos un día, o deja ambos vacíos.',
      applyAll: 'Aplicar a toda la semana',
      applyAllLabel: 'Recurso para toda la semana',
      dayNoResource: 'Sin recurso este día',
      plan: 'Plano',
      pickInPlan: 'Elegir en el plano',
      dayResourceHint:
        'Elige un recurso y marca sus días. Para asignar recursos distintos en días distintos, cambia de recurso y marca otros días.',
      dayUnassigned: 'Sin recurso',
      clearDay: 'Quitar {{day}}',
      multiResourceSummary: '{{name}} tendrá {{count}} recursos según el detalle por día.',
      resourceConflict: 'Ese recurso ya está asignado a otro empleado ese día.',
      resourceConflictDetail: '{{resource}} ya está asignado a otro empleado el {{days}}.',
      resourceSaveError: 'No se pudieron guardar las asignaciones fijas.',
      security: {
        title: 'Seguridad de la cuenta',
        note: 'Fase 1: contraseña temporal en pantalla. Fase 2: por email.',
        reset: 'Resetear contraseña',
      },
      parking: {
        title: 'Plaza fija asignada',
        hint: 'La asignación es indefinida hasta que se modifique. El empleado podrá liberar su plaza los días que no acuda.',
        space: 'Plaza',
        select: 'Selecciona una plaza',
        days: 'Días de la semana con plaza',
        perDayTitle: 'Plaza asignada por día',
        summary: '{{name}} tendrá la plaza {{space}} los {{days}} de forma indefinida.',
      },
      desk: {
        title: 'Puesto fijo asignado',
        hint: 'La asignación es indefinida hasta que se modifique. El empleado podrá liberar su puesto los días que no acuda.',
        desk: 'Puesto',
        select: 'Selecciona un puesto',
        days: 'Días de la semana con puesto',
        perDayTitle: 'Puesto asignado por día',
        summary: '{{name}} tendrá el puesto {{desk}} los {{days}} de forma indefinida.',
      },
      history: {
        title: 'Asignaciones fijas vigentes',
        hint: 'Resumen de solo lectura de la plaza y el puesto fijos actuales del empleado.',
        none: 'Sin asignación fija.',
        account: 'Datos de la cuenta',
        authOrigin: 'Origen de autenticación',
        createdAt: 'Alta',
        updatedAt: 'Última modificación',
      },
      save: 'Guardar',
      cancel: 'Cancelar',
      close: 'Cerrar',
      required: 'Este campo es obligatorio',
      invalidEmail: 'Introduce un email válido',
      duplicateLogin: 'Ya existe un empleado con ese usuario',
      duplicateEmail: 'Ya existe un empleado con ese email',
      genericError: 'No se pudo guardar el empleado.',
    },
    reset: {
      title: 'Restablecer contraseña',
      intro: 'Se ha generado una contraseña temporal para {{name}}.',
      onceWarning: '⚠️ Se muestra una sola vez. Cópiala y entrégala de forma segura.',
      tempPasswordLabel: 'Contraseña temporal',
      copy: 'Copiar',
      copied: 'Copiada',
      done: 'Hecho',
      mustChangeNote: 'El empleado deberá cambiarla en su primer inicio de sesión.',
      phase2Note: 'En Fase 2 la contraseña se enviará por email y no se mostrará aquí.',
      confirm: 'Generar contraseña temporal',
      close: 'Cerrar',
      confirmBody: '¿Restablecer la contraseña de {{name}}?',
      error: 'No se pudo restablecer la contraseña.',
    },
  },
  parkingSpaces: {
    title: 'Plazas',
    eyebrow: 'Administración',
    description: 'Configura el total de plazas del parking y gestiona su estado.',
    navLabel: 'Plazas',
    new: 'Nueva plaza',
    searchLabel: 'Buscar plaza',
    searchPlaceholder: 'Buscar plaza…',
    filterLabel: 'Filtrar por estado',
    filter: {
      all: 'Todas',
      active: 'Activas',
      inactive: 'Inactivas',
    },
    filterFloorLabel: 'Filtrar por planta',
    floorFilter: {
      all: 'Todas las plantas',
    },
    stats: {
      total: 'Total',
      active: 'Activas',
      inactive: 'Inactivas',
      unit: 'plazas',
      ofTotal: 'de {{total}}',
    },
    floorOption: 'Planta {{floor}}',
    floorValue: 'Planta {{floor}}',
    columns: {
      label: 'Plaza',
      floor: 'Planta',
      status: 'Estado',
      actions: 'Acciones',
    },
    status: {
      active: 'Activa',
      inactive: 'Inactiva',
    },
    actions: {
      edit: 'Editar',
      activate: 'Activar',
      deactivate: 'Desactivar',
    },
    empty: 'No hay plazas que mostrar.',
    loadError: 'No se pudieron cargar las plazas.',
    errors: {
      toggle: 'No se pudo cambiar el estado de la plaza.',
      hasFutureAssignments:
        'No se puede desactivar la plaza porque tiene asignaciones vigentes o futuras (de empleados o visitantes). Reasígnalas o libéralas antes de desactivarla.',
    },
    pagination: {
      previous: 'Anterior',
      next: 'Siguiente',
      pageInfo: 'Página {{page}} de {{total}}',
    },
    configure: {
      title: 'Configuración del parking',
      hint: 'Ajusta el número total de plazas. El histórico se conserva.',
      totalLabel: 'Número total de plazas',
      apply: 'Aplicar',
      applied: 'Configuración aplicada.',
      invalidTotal: 'Introduce un número entero mayor o igual que 0.',
    },
    form: {
      createTitle: 'Nueva plaza',
      editTitle: 'Editar plaza',
      number: 'Número de la plaza',
      numberHint: 'Entero único ≥ 1000. La planta es el número ÷ 1000.',
      floorLabel: 'Planta',
      floorHint: 'Se calcula automáticamente a partir del número.',
      statusLabel: 'Estado',
      active: 'Plaza activa',
      activeHint: 'Solo las plazas activas son reservables.',
      holderInfo:
        'La asignación de un titular fijo y de días se hace después, desde la ficha del empleado.',
      save: 'Guardar',
      cancel: 'Cancelar',
      required: 'Este campo es obligatorio',
      numberInvalid: 'Introduce un número entero mayor o igual que 1000.',
      duplicateNumber: 'Ya existe una plaza con ese número',
      genericError: 'No se pudo guardar la plaza.',
    },
  },
  desks: {
    title: 'Puestos',
    eyebrow: 'Administración',
    description: 'Gestiona los puestos de oficina, su categoría y activación.',
    navLabel: 'Puestos',
    new: 'Nuevo puesto',
    searchLabel: 'Buscar puesto',
    searchPlaceholder: 'Buscar puesto…',
    filterLabel: 'Filtrar por estado',
    filter: {
      all: 'Todos',
      active: 'Activos',
      inactive: 'Inactivos',
    },
    stats: {
      total: 'Total',
      active: 'Activos',
      inactive: 'Inactivos',
      unit: 'puestos',
      ofTotal: 'de {{total}}',
    },
    columns: {
      number: 'Número',
      category: 'Categoría',
      status: 'Estado',
      actions: 'Acciones',
    },
    category: {
      STANDARD: 'Estándar',
      EXECUTIVE: 'Dirección',
    },
    status: {
      active: 'Activo',
      inactive: 'Inactivo',
    },
    actions: {
      edit: 'Editar',
      activate: 'Activar',
      deactivate: 'Desactivar',
      delete: 'Borrar',
    },
    empty: 'No hay puestos que mostrar.',
    loadError: 'No se pudieron cargar los puestos.',
    toast: {
      created: 'Puesto creado',
      updated: 'Puesto actualizado',
      deleted: 'Puesto borrado',
    },
    form: {
      createTitle: 'Nuevo puesto',
      editTitle: 'Editar puesto',
      number: 'Número del puesto',
      numberImmutable: 'El número no se puede cambiar.',
      category: 'Categoría',
      statusLabel: 'Estado',
      active: 'Puesto activo',
      activeHint: 'Solo los puestos activos son reservables.',
      holderInfo:
        'La asignación de un titular fijo y de días se hace después, desde la ficha del empleado.',
      save: 'Guardar',
      cancel: 'Cancelar',
      required: 'Este campo es obligatorio',
      numberRange: 'El número debe ser un entero mayor que 0',
      duplicateNumber: 'Ya existe un puesto con ese número',
      genericError: 'No se pudo guardar el puesto.',
      delete: 'Borrar puesto',
      deleteConfirm: '¿Borrar este puesto? Esta acción no se puede deshacer.',
      deleteConfirmYes: 'Sí, borrar',
      deleteHasHistory:
        'No se puede borrar: el puesto tiene reservas o historial. Desactívalo en su lugar.',
      deleteError: 'No se pudo borrar el puesto.',
    },
    errors: {
      toggle: 'No se pudo cambiar el estado del puesto.',
      hasFutureAssignments:
        'No se puede desactivar el puesto porque tiene asignaciones vigentes o futuras (de empleados o visitantes). Reasígnalas o libéralas antes de desactivarlo.',
    },
  },
  fixedAssignments: {
    title: 'Asignaciones fijas',
    navLabel: 'Asignaciones fijas',
    new: 'Nueva asignación',
    columns: {
      employee: 'Empleado',
      type: 'Tipo',
      space: 'Recurso',
      days: 'Días',
      actions: 'Acciones',
    },
    weekdays: {
      1: 'Lunes',
      2: 'Martes',
      3: 'Miércoles',
      4: 'Jueves',
      5: 'Viernes',
      6: 'Sábado',
      7: 'Domingo',
    },
    empty: 'No hay asignaciones fijas activas.',
    loadError: 'No se pudieron cargar las asignaciones fijas.',
    pagination: {
      previous: 'Anterior',
      next: 'Siguiente',
      pageInfo: 'Página {{page}} de {{total}}',
    },
    actions: {
      edit: 'Editar',
      revoke: 'Revocar',
    },
    form: {
      createTitle: 'Nueva asignación fija',
      editTitle: 'Editar asignación fija',
      employee: 'Empleado',
      resource: 'Tipo de recurso',
      resourceParking: 'Plaza',
      resourceDesk: 'Puesto',
      space: 'Plaza',
      desk: 'Puesto',
      days: 'Días de la semana',
      selectEmployee: 'Selecciona un empleado',
      selectSpace: 'Selecciona una plaza',
      selectDesk: 'Selecciona un puesto',
      save: 'Guardar',
      cancel: 'Cancelar',
      requiredEmployee: 'Selecciona un empleado',
      requiredSpace: 'Selecciona una plaza',
      requiredDesk: 'Selecciona un puesto',
      requiredDays: 'Selecciona al menos un día',
    },
    revoke: {
      title: 'Revocar asignación fija',
      body: '¿Revocar la asignación fija de {{name}}? Se marcará como inactiva sin borrar el histórico.',
      confirm: 'Revocar',
      cancel: 'Cancelar',
    },
    errors: {
      invalidDays: 'Días inválidos. Selecciona días entre lunes y domingo.',
      conflict: 'La plaza o el empleado ya tiene una asignación fija ese día.',
      generic: 'No se pudo guardar la asignación fija.',
    },
    mine: {
      title: 'Mis asignaciones fijas',
      navLabel: 'Mis asignaciones',
      eyebrow: 'Mi espacio',
      description: 'Tus recursos fijos y los días que los tienes reservados.',
      empty: 'No tienes asignaciones fijas.',
      loadError: 'No se pudieron cargar tus asignaciones fijas.',
      // Etiqueta generica para una plaza fija: no hay endpoint EMPLOYEE-safe para
      // resolver el numero real de la plaza (catalogo de plazas es solo ADMIN).
      genericParkingLabel: 'Plaza fija',
      columns: {
        space: 'Plaza',
        resource: 'Recurso',
        days: 'Días',
      },
    },
  },
  requests: {
    status: {
      PENDING: 'Pendiente',
      APPROVED: 'Aprobada',
      REJECTED: 'Rechazada',
      CANCELLED: 'Cancelada',
      RELEASED: 'Liberada',
    },
    resourceType: {
      PARKING: 'Plaza',
      DESK: 'Puesto',
    },
    reasonCodes: {
      NO_AVAILABILITY: 'Sin disponibilidad',
      OUTSIDE_POLICY: 'Fuera de política',
      OTHER: 'Otro motivo',
    },
    pagination: {
      previous: 'Anterior',
      next: 'Siguiente',
      pageInfo: 'Página {{page}} de {{total}}',
    },
    mine: {
      title: 'Mis solicitudes',
      navLabel: 'Mis solicitudes',
      eyebrow: 'Mi espacio',
      description: 'Historial de tus solicitudes de plaza y puesto, con su estado.',
      new: 'Nueva solicitud',
      empty: 'No tienes solicitudes.',
      emptyMonth: 'No tienes solicitudes este mes.',
      prevMonth: 'Mes anterior',
      nextMonth: 'Mes siguiente',
      filterStatus: 'Filtrar por estado',
      filterAll: 'Todas',
      filterResource: 'Filtrar por recurso',
      filterAllResources: 'Todos los recursos',
      loadError: 'No se pudieron cargar tus solicitudes.',
      columns: {
        date: 'Día reservado',
        status: 'Estado',
        resource: 'Recurso',
        space: 'Plaza/Puesto',
        actions: 'Acciones',
      },
      // Numero real del recurso asignado que ve el empleado (no el id interno).
      resourceLabel: {
        parking: 'Plaza {{number}}',
        parkingWithFloor: 'Plaza {{number}} · Planta {{floor}}',
        desk: 'Puesto {{number}}',
      },
      cancel: 'Cancelar',
      // Acción por estado en la fila: cancelar la solicitud (PENDING) o liberar el
      // recurso ya asignado (APPROVED).
      cancelRequest: 'Cancelar solicitud',
      release: 'Liberar',
      // Fecha en la que se realizó la solicitud (secundaria bajo el día reservado).
      requestedOn: 'Solicitado el {{date}}',
      created: 'Solicitud creada.',
      createdApproved: 'Solicitud aprobada: recurso asignado al instante.',
      createdWaitlisted: 'En lista de espera: te avisaremos en cuanto se libere un recurso.',
      // Distintivo de una solicitud PENDING nacida sin hueco (capability
      // request-waitlist). Nunca muestra posición numérica en la cola.
      waitlisted: 'En lista de espera',
    },
    inbox: {
      title: 'Solicitudes',
      navLabel: 'Solicitudes',
      eyebrow: 'Operativa',
      description: 'Resuelve las peticiones de plaza y puesto por orden de llegada.',
      pendingBadge: '{{count}} solicitudes pendientes',
      empty: 'No hay solicitudes pendientes.',
      loadError: 'No se pudieron cargar las solicitudes pendientes.',
      tabs: {
        pending: 'Pendientes',
        approved: 'Aprobadas',
        rejected: 'Rechazadas',
        all: 'Todas',
        ariaLabel: 'Filtrar solicitudes por estado',
      },
      search: {
        label: 'Buscar empleado',
        placeholder: 'Buscar empleado…',
      },
      emptyByStatus: 'No hay solicitudes en este estado.',
      columns: {
        employee: 'Empleado',
        date: 'Fecha solicitada',
        day: 'Día',
        created: 'Creada',
        status: 'Estado',
        actions: 'Acciones',
      },
      approve: 'Aprobar',
      reject: 'Rechazar',
      cancel: 'Cancelar',
    },
    create: {
      title: 'Nueva solicitud',
      date: 'Fecha de la solicitud',
      hint: 'Puedes solicitar cualquier día desde hoy en adelante.',
      resources: 'Recursos a solicitar',
      resourceParking: 'Plaza de parking',
      resourceDesk: 'Puesto de trabajo',
      submit: 'Enviar solicitud',
      cancel: 'Cancelar',
      alreadyReserved: 'Ya tienes reserva de {{what}} para este día.',
      alreadyReservedWhat: {
        parking: 'plaza de parking',
        desk: 'puesto de trabajo',
        both: 'plaza y puesto',
      },
      fixedHint: 'Si sigue libre, se te reasignará tu sitio fijo de ese día.',
      willAssign: 'Se te asignará {{resource}}',
      legend: {
        parking: 'Ya tienes plaza',
        desk: 'Ya tienes puesto',
      },
      // Aviso (gris) de los días seleccionados que ya tienes reservados.
      alreadyHave: {
        parking: 'Ya tienes plaza reservada el {{dates}}.',
        desk: 'Ya tienes puesto reservado el {{dates}}.',
        skipTail: 'Esos días se omitirán al enviar.',
        blockTail: 'Elige otro día u otro recurso para continuar.',
      },
      selectedCount: '{{count}} días seleccionados',
      maxDays: 'Máximo {{max}} días por reserva. Para más días, contacta con el administrador.',
      pickDatePrompt: 'Elige uno o varios días en el calendario',
      selectedDateLabel: 'Día elegido',
      today: 'Hoy',
      calendar: {
        prev: 'Mes anterior',
        next: 'Mes siguiente',
      },
      card: {
        reservedParking: 'Ya tienes plaza este día',
        reservedDesk: 'Ya tienes puesto este día',
        available: '{{count}} libre(s) {{when}}',
        today: 'hoy',
        onDay: 'el {{day}}',
        none: 'Sin disponibilidad',
        needDate: 'Elige una fecha',
        checking: 'Comprobando…',
        selected: 'Seleccionada',
      },
      chooseDesk: 'Seleccionar puesto',
      changeDesk: 'Cambiar puesto',
      removeDesk: 'Quitar',
      chosenDesk: 'Puesto elegido: {{number}}',
      chosenDeskPreferenceNote:
        'Es una preferencia: en modo manual el ADMIN resuelve la asignación final y podría asignar otro puesto.',
      chooseDeskDateHint: 'Elige primero una fecha (hoy o futura) para seleccionar un puesto.',
      requiredDate: 'Selecciona una fecha.',
      requiredResource: 'Selecciona al menos un recurso (plaza o puesto).',
      outsideWindow: 'La fecha no puede ser anterior a hoy.',
      availability: {
        loading: 'Comprobando disponibilidad…',
        count: '{{count}} disponible(s) para esta fecha',
      },
      automaticNotice: 'Se confirmará al instante.',
      manualNotice: 'Quedará pendiente de aprobación. Tu plaza o puesto exacto puede cambiar.',
      // Lista de espera (capability request-waitlist): aviso honesto cuando la
      // disponibilidad del recurso es 0 y confirmación tras el 409 NO_AVAILABILITY.
      waitlist: {
        resourceNoun: {
          PARKING: 'plazas',
          DESK: 'puestos',
        },
        noneHint:
          'No quedan {{resource}} libres este día. Suelen liberarse (ausencias, vacaciones, cancelaciones): apúntate y te avisaremos en cuanto haya una.',
        join: 'Apuntarme a la lista de espera',
        joined: 'Apuntado: te avisaremos si se libera',
        retryPrompt:
          'No hay hueco para ese recurso ahora mismo. ¿Quieres apuntarte a la lista de espera? Te avisaremos en cuanto se libere.',
        retryConfirm: 'Apuntarme a la lista de espera',
        retryDismiss: 'No, gracias',
      },
    },
    pendingBanner: {
      message: 'Pendiente de confirmación · la ubicación podría cambiar',
      resend: 'Reenviar solicitud',
      resent: 'Solicitud reenviada a los administradores.',
      remaining: 'Podrás reavisar en {{hours}} h',
    },
    cancel: {
      title: 'Cancelar solicitud',
      body: '¿Cancelar tu solicitud del {{date}}?',
      confirm: 'Cancelar solicitud',
      keep: 'No cancelar',
      done: 'Solicitud cancelada. El recurso queda libre ese día.',
    },
    releaseRequest: {
      title: 'Liberar reserva',
      body: '¿Liberar tu reserva del {{date}}? Quedará disponible para otra persona ese día.',
      confirm: 'Liberar',
      keep: 'No liberar',
    },
    adminCancel: {
      title: 'Liberar cancelando la solicitud',
      summary: 'Resumen de la liberación',
      employee: 'Empleado',
      resource: 'Recurso',
      date: 'Fecha',
      reason: 'Motivo',
      reasonHint: 'Obligatorio (mínimo 5 caracteres). Queda registrado en la auditoría.',
      submit: 'Liberar',
      cancel: 'Cancelar',
      requiredReason: 'Indica el motivo (mínimo 5 caracteres).',
      cancelled: 'Solicitud cancelada. El recurso queda libre esa fecha.',
      errors: {
        reason: 'Revisa el motivo (5 a 500 caracteres).',
      },
    },
    approve: {
      title: 'Aprobar solicitud',
      titleParking: 'Solicitud de plaza',
      titleDesk: 'Solicitud de puesto',
      space: 'Plaza a asignar',
      resourceParking: 'Plaza disponible',
      resourceDesk: 'Puesto disponible',
      selectSpace: 'Selecciona una plaza',
      selectParking: 'Selecciona una plaza',
      selectDesk: 'Selecciona un puesto',
      loadingResources: 'Cargando disponibilidad…',
      note: 'Nota de aprobación (opcional)',
      submit: 'Aprobar',
      cancel: 'Cancelar',
      requiredSpace: 'Selecciona una plaza.',
      requiredParking: 'Selecciona una plaza.',
      requiredDesk: 'Selecciona un puesto.',
      context: {
        employee: 'Empleado',
        department: 'Departamento',
        noDepartment: 'Sin departamento',
        day: 'Día solicitado',
        status: 'Estado',
        resourceType: 'Tipo de recurso',
      },
      freeResources: '{{count}} libre(s) ese día',
      emailNotice: 'Al aprobar, se enviará un email automático a {{email}}.',
      emailNoticeGeneric: 'Al aprobar, se enviará un email automático al empleado.',
    },
    reject: {
      title: 'Rechazar solicitud',
      reason: 'Motivo del rechazo',
      selectReason: 'Selecciona un motivo',
      detail: 'Comentario adicional',
      detailHint: 'Obligatorio (mínimo 5 caracteres) cuando el motivo es «Otro motivo».',
      submit: 'Confirmar rechazo',
      cancel: 'Cancelar',
      requiredReason: 'Selecciona un motivo.',
      requiredDetail: 'Indica el detalle del motivo (mínimo 5 caracteres).',
      intro: 'Estás a punto de rechazar la solicitud de {{resource}} de {{name}} para el {{date}}.',
      emailNotice: 'El empleado recibirá un email con el motivo del rechazo.',
    },
    cancelApproved: {
      title: 'Cancelar solicitud aprobada',
      submit: 'Confirmar cancelación',
      intro:
        'Vas a cancelar la solicitud aprobada de {{resource}} de {{name}} para el {{date}}. El recurso quedará libre.',
      emailNotice: 'El empleado recibirá un email informándole de la cancelación.',
    },
    errors: {
      window: 'La fecha no puede ser anterior a hoy.',
      duplicate: 'Ya tienes una solicitud pendiente para esa fecha.',
      alreadyResolved: 'La solicitud ya no está pendiente.',
      unavailable: 'La plaza no está disponible para esa fecha.',
      noAvailability: 'No hay plazas disponibles para esa fecha.',
      rejectInvalid: 'Revisa el motivo del rechazo.',
      resendForbidden: 'No puedes reenviar el aviso de una solicitud que no es tuya.',
      resendTooSoon: 'Aún no ha pasado tiempo suficiente desde el último aviso.',
      generic: 'No se pudo completar la operación.',
    },
  },
  releases: {
    hub: {
      navLabel: 'Liberar',
      title: 'Liberar',
      eyebrow: 'Operativa',
      description: 'Libera plazas y puestos por empleado o por fecha, y consulta el historial.',
      tabs: {
        byEmployee: 'Por empleado',
        byDate: 'Por fecha',
        history: 'Historial',
      },
      searchPerson: 'Buscar empleado…',
      noPerson: 'Sin resultados',
    },
    history: {
      title: 'Mis liberaciones administrativas',
      eyebrow: 'Operativa',
      description: 'Historial de las liberaciones administrativas que has creado.',
      loadError: 'No se pudo cargar el historial de liberaciones.',
      empty: 'Aún no has creado liberaciones administrativas.',
      columns: {
        date: 'Fecha',
        type: 'Recurso',
        resource: 'Identificador',
        reason: 'Motivo',
      },
    },
    type: {
      VOLUNTARY: 'Voluntaria',
      ADMINISTRATIVE: 'Administrativa',
    },
    pagination: {
      previous: 'Anterior',
      next: 'Siguiente',
      pageInfo: 'Página {{page}} de {{total}}',
    },
    mine: {
      title: 'Mis liberaciones',
      navLabel: 'Mis liberaciones',
      eyebrow: 'Mi espacio',
      description: 'Días que has liberado tu recurso fijo para que otro lo use.',
      empty: 'No tienes liberaciones.',
      loadError: 'No se pudieron cargar tus liberaciones.',
      columns: {
        date: 'Fecha',
        type: 'Tipo',
        space: 'Plaza',
        actions: 'Acciones',
      },
      cancel: 'Cancelar',
      cancelled: 'Liberación anulada.',
    },
    release: {
      action: 'Liberar',
      title: 'Liberar mi plaza',
      space: 'Plaza',
      date: 'Fecha a liberar',
      hint: 'Solo puedes liberar días presentes o futuros. La plaza quedará disponible para otro empleado.',
      submit: 'Liberar',
      cancel: 'Cancelar',
      requiredDate: 'Selecciona una fecha.',
      created: 'Plaza liberada.',
      fixedResource: 'Tu recurso fijo es {{label}}.',
      summary: {
        title: 'Resumen',
        resource: 'Plaza / Puesto',
        day: 'Día',
        type: 'Tipo',
        voluntary: 'VOLUNTARIA',
        pendingDay: 'Selecciona un día',
        availableNote: 'Ese día tu recurso quedará disponible para que otro empleado lo solicite.',
      },
    },
    cancel: {
      title: 'Anular liberación',
      body: '¿Anular tu liberación del {{date}}? Tu plaza volverá a estar reservada para ti ese día.',
      confirm: 'Anular liberación',
      keep: 'No anular',
    },
    admin: {
      title: 'Liberación administrativa',
      eyebrow: 'Operativa',
      description: 'Libera el recurso fijo de un empleado para una fecha concreta.',
      navLabel: 'Liberaciones',
      intro:
        'Libera el recurso fijo de un empleado para una fecha concreta cuando no vaya a acudir. Requiere un motivo.',
      new: 'Nueva liberación',
      employee: 'Empleado',
      selectEmployee: 'Selecciona un empleado',
      space: 'Plaza',
      selectSpace: 'Selecciona una plaza',
      date: 'Fecha a liberar',
      reason: 'Motivo',
      reasonHint: 'Obligatorio. Queda registrado en la auditoría.',
      submit: 'Liberar',
      cancel: 'Cancelar',
      requiredEmployee: 'Selecciona un empleado.',
      requiredSpace: 'Selecciona una plaza.',
      requiredDate: 'Selecciona una fecha.',
      requiredReason: 'Indica el motivo de la liberación.',
      created: 'Liberación administrativa creada.',
    },
    byDate: {
      navLabel: 'Liberar por fecha',
      title: 'Liberar por fecha',
      eyebrow: 'Operativa',
      description: 'Recursos ocupados por fecha y su titular, para liberarlos uno a uno.',
      intro:
        'Elige una fecha para ver los recursos ocupados y por qué empleado. Libéralos uno a uno o marca varios y libéralos a la vez.',
      dateLabel: 'Fecha',
      prevDay: 'Día anterior',
      nextDay: 'Día siguiente',
      loadError: 'No se pudo cargar la ocupación de esa fecha.',
      empty: 'No hay recursos ocupados esa fecha.',
      release: 'Liberar',
      selectAll: 'Seleccionar todo',
      selectedCount: '{{count}} seleccionada(s)',
      releaseSelected: 'Liberar seleccionadas ({{count}})',
      batchTitle: '¿Liberar {{count}} recurso(s)?',
      batchBody: 'Se liberarán los {{count}} recursos seleccionados para esa fecha. Quedará registrado en la auditoría.',
      batchReasonLabel: 'Motivo',
      batchReasonHint: 'Obligatorio (mínimo 5 caracteres). Se aplica a todas.',
      batchError: 'No se pudieron liberar los recursos seleccionados.',
      summary: 'Resumen de la liberación',
      resourcePark: 'Plaza {{number}} · Planta {{floor}}',
      resourceDesk: 'Puesto {{number}}',
      resourceType: {
        PARKING: 'Plaza',
        DESK: 'Puesto',
      },
      origin: {
        FIXED_ASSIGNMENT: 'Asignación fija',
        REQUEST_APPROVED: 'Solicitud aprobada',
      },
      columns: {
        resource: 'Recurso',
        type: 'Tipo',
        employee: 'Empleado',
        origin: 'Origen',
        actions: 'Acciones',
      },
    },
    employeeWeek: {
      description:
        'Selecciona un empleado, revisa sus reservas de la semana y libera las que no vaya a usar.',
      intro:
        'Elige un empleado para ver sus reservas (plaza y puesto) por semana, marca una o varias y libéralas con un único motivo.',
      employee: 'Empleado',
      selectEmployee: 'Selecciona un empleado',
      selectEmployeePrompt: 'Selecciona un empleado para ver sus reservas de la semana.',
      loading: 'Cargando reservas del empleado…',
      loadError: 'No se pudieron cargar las reservas del empleado.',
      empty: 'Este empleado no tiene reservas esta semana.',
      reason: 'Motivo',
      reasonHint:
        'Obligatorio (mínimo 5 caracteres). Se aplica a todas las reservas marcadas y queda registrado en la auditoría.',
      requiredReason: 'Indica el motivo (mínimo 5 caracteres).',
      requiredSelection: 'Marca al menos una reserva para liberar.',
      release: 'Liberar',
      releaseBatch: 'Liberar reservas',
      releaseSelected: 'Liberar seleccionadas ({{count}})',
      batchTitle: '¿Liberar {{count}} reserva(s)?',
      modeLabel: 'Modo de liberación',
      modeWeek: 'Semana',
      modeRange: 'Rango',
      cancel: 'Cancelar',
      weekNumber: 'Semana {{week}}',
      selectedCount: '{{count}} reserva(s) seleccionada(s)',
      selectedCountLabel: 'reserva(s) seleccionada(s)',
      selectedDays: '{{count}} día(s)',
      resourceKind: {
        PARKING: 'Plaza de parking',
        DESK: 'Puesto de oficina',
      },
      feedback: {
        released: '{{count}} reserva(s) liberada(s).',
        partial: '{{released}} reserva(s) liberada(s), {{failed}} con error.',
        failedAll: 'No se pudo liberar ninguna reserva.',
      },
    },
    range: {
      from: 'Desde',
      to: 'Hasta',
      empty: 'Este empleado no tiene reservas en ese rango.',
      preset: {
        week1: '1 semana',
        week2: '2 semanas',
        month1: '1 mes',
      },
    },
    errors: {
      window: 'La fecha debe ser hoy o futura.',
      conflict: 'El recurso no tiene asignación fija ese día o ya está liberado.',
      forbidden: 'No puedes anular una liberación que no es tuya.',
      generic: 'No se pudo completar la operación.',
    },
  },
  visitors: {
    title: 'Visitantes',
    eyebrow: 'Administración',
    description: 'Fichas de visitante y reservas de plaza para externos.',
    navLabel: 'Visitantes',
    emailNote: 'Las reservas de visitante no generan notificaciones por email.',
    tabs: {
      visitors: 'Fichas de visitante',
      reservations: 'Reservas futuras',
    },
    searchPlaceholder: 'Buscar por DNI, nombre o matrícula…',
    searchLabel: 'Buscar visitante',
    newVisitor: 'Nuevo visitante',
    newReservation: 'Nueva reserva',
    empty: 'No hay visitantes que mostrar.',
    loadError: 'No se pudieron cargar los visitantes.',
    columns: {
      name: 'Visitante',
      nationalId: 'DNI',
      licensePlate: 'Matrícula',
      company: 'Empresa',
      usualReason: 'Motivo habitual',
      actions: 'Acciones',
    },
    actions: {
      view: 'Ver',
      edit: 'Editar',
      reserve: 'Reservar',
    },
    pagination: {
      previous: 'Anterior',
      next: 'Siguiente',
      pageInfo: 'Página {{page}} de {{total}}',
    },
    form: {
      createTitle: 'Nuevo visitante',
      editTitle: 'Editar visitante',
      tabs: {
        details: 'Detalles',
        vehicles: 'Vehículos',
      },
      firstName: 'Nombre',
      lastName: 'Apellidos',
      nationalId: 'DNI / Documento',
      company: 'Empresa',
      usualReason: 'Motivo habitual',
      save: 'Guardar',
      cancel: 'Cancelar',
      required: 'Este campo es obligatorio',
      duplicateNationalId: 'Ya existe un visitante con ese documento',
      genericError: 'No se pudo guardar el visitante.',
    },
    vehicles: {
      tabLabel: 'Vehículos',
      saveFirst: 'Guarda primero el visitante para poder añadir vehículos.',
      empty: 'Este visitante no tiene vehículos.',
      loadError: 'No se pudieron cargar los vehículos.',
      add: 'Añadir vehículo',
      edit: 'Editar',
      editTitle: 'Editar vehículo',
      delete: 'Borrar',
      confirmDelete: 'Sí, borrar',
      deleteTitle: 'Borrar vehículo',
      deleteBody: '¿Seguro que quieres borrar el vehículo {{plate}}? Esta acción no se puede deshacer.',
      saveVehicle: 'Guardar vehículo',
      plateRequired: 'La matrícula es obligatoria.',
      plateTaken: 'Este visitante ya tiene un vehículo con esa matrícula.',
      saveError: 'No se pudo guardar el vehículo.',
      columns: {
        brand: 'Marca',
        model: 'Modelo',
        plate: 'Matrícula',
        color: 'Color',
        actions: 'Acciones',
      },
      form: {
        plate: 'Matrícula *',
        brand: 'Marca',
        model: 'Modelo',
        color: 'Color',
      },
    },
    detail: {
      title: 'Ficha de visitante',
      firstName: 'Nombre',
      lastName: 'Apellidos',
      nationalId: 'DNI / Documento',
      licensePlate: 'Matrícula',
      company: 'Empresa',
      usualReason: 'Motivo habitual',
      createdAt: 'Alta',
      none: '—',
      close: 'Cerrar',
      loadError: 'No se pudo cargar la ficha del visitante.',
    },
    reservations: {
      empty: 'No hay reservas de visita.',
      loadError: 'No se pudieron cargar las reservas de visita.',
      cancel: 'Anular',
      columns: {
        date: 'Fecha',
        visitor: 'Visitante',
        space: 'Plaza',
        notes: 'Observaciones',
        actions: 'Acciones',
      },
      create: {
        title: 'Nueva reserva de visita',
        visitor: 'Visitante',
        selectVisitor: 'Selecciona un visitante',
        date: 'Fecha de la reserva',
        space: 'Plaza',
        selectSpace: 'Selecciona una plaza',
        spaceHint: 'Solo se muestran las plazas activas.',
        notes: 'Observaciones (opcional)',
        banner: 'La plaza quedará ocupada esa fecha. No se envían emails.',
        submit: 'Crear reserva',
        cancel: 'Cancelar',
        requiredVisitor: 'Selecciona un visitante.',
        requiredDate: 'Selecciona una fecha.',
        requiredSpace: 'Selecciona una plaza.',
      },
      cancelModal: {
        title: 'Anular reserva de visita',
        body: '¿Anular la reserva del {{date}}? La plaza volverá a estar disponible.',
        confirm: 'Anular reserva',
        keep: 'No anular',
      },
      errors: {
        occupied: 'La plaza ya está ocupada para esa fecha.',
        validation: 'Revisa los datos de la reserva.',
        past: 'No se puede anular una reserva pasada.',
        generic: 'No se pudo completar la operación.',
      },
    },
  },
  floorPlan: {
    navLabel: 'Plano',
    title: 'Plano de puestos',
    eyebrow: 'Mi espacio',
    description: 'Consulta los puestos libres y solicita el tuyo directamente sobre el plano.',
    dateLabel: 'Fecha',
    invalidDate: 'Introduce una fecha válida.',
    loadError: 'No se pudo cargar el plano.',
    outsideWindow: 'La fecha no puede ser anterior a hoy.',
    editPositions: 'Editar posiciones',
    editHint: 'Arrastra los marcadores para colocar cada puesto; pulsa «Guardar cambios» para aplicarlos o «Cancelar» para descartarlos.',
    savePositions: 'Guardar posiciones',
    saveChanges: 'Guardar cambios',
    saveError: 'No se pudieron guardar las posiciones. Inténtalo de nuevo.',
    positionsSaved: 'Posiciones guardadas.',
    requestSuccess: 'Solicitud creada. Queda pendiente de aprobación.',
    requestConflict: 'El puesto ya no está disponible para esa fecha.',
    imageAlt: 'Plano de la planta de oficina',
    minimap: { label: 'Minimapa: arrastra el recuadro para moverte por el plano' },
    fullscreen: { enter: 'Ver el plano a pantalla completa', exit: 'Salir de pantalla completa' },
    unplacedTitle: 'Puestos sin posición',
    unplacedHint: 'Estos puestos aún no están colocados en el plano.',
    deskNumber: 'Puesto {{number}}',
    markerLabel: 'Puesto {{number}} — {{state}}',
    markerLabelSelected: 'Puesto {{number}} — {{state}} (seleccionado)',
    markerLabelFocused: 'Puesto {{number}} — {{state}} (resaltado)',
    legendLabel: 'Leyenda de estados',
    legendExecutive: 'Dirección',
    canvasLabel: 'Planta de oficina',
    tooltip: {
      free: 'Disponible · toca para reservar',
      released: 'Liberado hoy · disponible',
      mine: 'Tu puesto',
      requested: 'Solicitud pendiente',
      occupied: 'Ocupado',
      occupiedBy: 'Ocupado por {{name}}',
    },
    pin: {
      label: 'Ocupado por {{name}}',
      labelUnknown: 'Puesto ocupado',
      mine: 'Tu puesto',
    },
    states: {
      FREE: 'Libre',
      ASSIGNED: 'Ocupado',
      REQUESTED: 'Solicitado',
      MINE: 'Mi puesto',
      RELEASED: 'Liberado hoy',
    },
    datebar: {
      label: 'Cambiar día',
      previous: 'Día anterior',
      next: 'Día siguiente',
      today: 'Hoy',
      window: 'Puedes reservar desde hoy en adelante',
    },
    filters: {
      label: 'Filtros por estado',
      chip: '{{label}}: {{count}}',
    },
    zoom: {
      label: 'Controles de zoom',
      in: 'Acercar',
      out: 'Alejar',
      reset: 'Restablecer zoom',
      resetShort: 'Restablecer',
      level: 'Zoom {{percent}}%',
    },
    side: {
      label: 'Panel de puestos',
      title: 'Puestos',
      occupancyTitle: 'Ocupación del día',
      countersLabel: 'Ocupación por estado',
      searchLabel: 'Buscar puesto por número',
      searchPlaceholder: 'Buscar nº…',
      empty: 'Ningún puesto coincide con la búsqueda.',
      rowAction: 'Solicitar el nº {{number}}',
      assignAction: 'Asignar el nº {{number}}',
    },
    mobile: {
      title: 'Disponibles para solicitar',
      empty: 'No hay puestos libres para esta fecha.',
      request: 'Solicitar',
    },
    select: {
      title: 'Seleccionar puesto',
      hint: 'Pincha un puesto libre para elegirlo. No se creará la solicitud hasta que la envíes.',
      confirmation: 'Puesto {{number}} seleccionado',
      close: 'Cerrar',
    },
    confirmRequest: {
      title: 'Confirmar solicitud',
      body: 'Vas a solicitar el puesto {{number}} para el {{date}}.',
      confirm: 'Solicitar',
      cancel: 'Cancelar',
    },
  },
  availability: {
    navLabel: 'Disponibilidad',
    title: 'Disponibilidad por fecha',
    eyebrow: 'Operativa',
    description: 'Consulta los recursos libres (plaza o puesto) para una fecha concreta.',
    dateLabel: 'Fecha',
    resourceTypeLabel: 'Tipo de recurso',
    resourceType: {
      PARKING: 'Plaza',
      DESK: 'Puesto',
    },
    invalidDate: 'Introduce una fecha válida.',
    hint: 'Elige una fecha para ver los recursos disponibles.',
    loadError: 'No se pudo cargar la disponibilidad.',
    empty: {
      PARKING: 'No hay plazas disponibles para esta fecha.',
      DESK: 'No hay puestos disponibles para esta fecha.',
    },
    count: {
      PARKING: '{{count}} plaza(s) disponible(s)',
      DESK: '{{count}} puesto(s) disponible(s)',
    },
    columns: {
      space: 'Recurso',
      type: 'Tipo',
    },
  },
  calendar: {
    admin: {
      navLabel: 'Asignación semanal',
      title: 'Asignación semanal',
      eyebrow: 'Operativa',
      description: 'Estado de cada plaza, día a día, de un vistazo.',
    },
    toolbar: {
      previous: 'Semana anterior',
      next: 'Semana siguiente',
      today: 'Hoy',
      weekOf: 'Semana del {{date}}',
    },
    actions: {
      filter: 'Filtrar',
      export: 'Exportar',
    },
    summary: {
      spaces: 'Plazas activas',
      assignments: 'Asignaciones',
      releases: 'Liberadas',
      requests: 'Solicitudes',
    },
    weekNav: {
      week: 'Semana {{number}}',
    },
    export: {
      filename: 'asignacion-semanal',
    },
    space: 'Plaza',
    loadError: 'No se pudo cargar el calendario.',
    empty: 'No hay plazas configuradas.',
    states: {
      ASSIGNED: 'Asignada',
      RELEASED: 'Liberada',
      REQUEST_PENDING: 'Solicitud pendiente',
      REQUEST_APPROVED: 'Solicitud aprobada',
      VISITOR_RESERVATION: 'Visitante',
      FREE: 'Libre',
    },
    weekdaysShort: {
      '0': 'Dom',
      '1': 'Lun',
      '2': 'Mar',
      '3': 'Mié',
      '4': 'Jue',
      '5': 'Vie',
      '6': 'Sáb',
    },
    myWeek: {
      navLabel: 'Mi Semana',
      title: 'Mi Semana',
      eyebrow: 'Mi espacio',
      description: 'Tu semana de un vistazo: solicita o libera tu plaza día a día.',
      hero: {
        sectionLabel: 'Hoy y mañana',
        today: 'Hoy',
        tomorrow: 'Mañana',
        reserve: 'Reservar',
        noData: 'Nada reservado. Pídelo cuando quieras.',
      },
      weekTitle: 'Próximos días',
      prevWeek: 'Semana anterior',
      nextWeek: 'Semana siguiente',
      emptyWeek: 'No quedan más días esta semana. Usa las flechas para ver las siguientes.',
      viewMap: 'Plano',
      deskNotFound: 'No se pudo localizar tu puesto en el plano.',
      goToParking: 'Ir al parking',
      loadError: 'No se pudo cargar tu semana.',
      empty: 'No hay días que mostrar.',
      space: 'Plaza {{label}}',
      noSpace: 'Sin plaza',
      requestPending: 'Solicitud pendiente',
      requestAction: 'Solicitar',
      releaseAction: 'Liberar',
      cancelAction: 'Cancelar',
      resourceKind: {
        PARKING: 'Plaza de parking',
        DESK: 'Puesto de trabajo',
      },
      noResource: {
        PARKING: 'Sin plaza este día',
        DESK: 'Sin puesto este día',
      },
      released: {
        PARKING: 'Plaza de parking liberada.',
        DESK: 'Puesto de trabajo liberado.',
      },
      resourceLabel: {
        PARKING: 'Plaza {{label}}',
        DESK: 'Puesto {{label}}',
      },
      states: {
        ASSIGNED: 'Asignado',
        RELEASED: 'Liberado',
        REQUEST_PENDING: 'Solicitud pendiente',
        FREE: 'Sin reservar',
      },
      heroShield: {
        yourParking: 'Tu plaza',
        yourDesk: 'Tu puesto',
        kindParking: 'Parking',
        kindDesk: 'Puesto',
        fixed: 'Fija',
        kindLabelParking: 'Plaza',
        kindLabelDesk: 'Puesto',
        noParking: 'Sin plaza',
        noDesk: 'Sin puesto',
      },
    },
  },
  audit: {
    title: 'Auditoría de acciones',
    eyebrow: 'Trazabilidad',
    description: 'Registro de acciones relevantes por actor, tipo y rango de fechas.',
    navLabel: 'Auditoría',
    retentionNote:
      'Los registros de auditoría se conservan durante 2 años y luego se purgan automáticamente.',
    systemActor: 'Sistema',
    filters: {
      actor: 'ID de empleado (actor)',
      action: 'Acción',
      from: 'Desde',
      to: 'Hasta',
    },
    columns: {
      occurredAt: 'Fecha',
      actor: 'Actor',
      action: 'Acción',
      entity: 'Entidad',
      details: 'Detalle',
    },
    empty: 'No hay entradas de auditoría que mostrar.',
    loadError: 'No se pudo cargar la auditoría.',
    eventsCount_one: '{{count}} evento',
    eventsCount_other: '{{count}} eventos',
    inRange: 'en el rango seleccionado',
    allTime: 'histórico completo',
    pagination: {
      previous: 'Anterior',
      next: 'Siguiente',
      pageInfo: 'Página {{page}} de {{total}}',
    },
    errors: {
      window: 'La fecha «Desde» debe ser anterior o igual a la fecha «Hasta».',
    },
  },
  loginLogs: {
    title: 'Accesos (login)',
    eyebrow: 'Trazabilidad',
    description: 'Intentos de inicio de sesión correctos y fallidos por fecha.',
    navLabel: 'Accesos',
    retentionNote:
      'Los intentos de inicio de sesión se conservan durante 2 años y luego se purgan automáticamente.',
    filters: {
      result: 'Resultado',
      allResults: 'Todos los resultados',
      from: 'Desde',
      to: 'Hasta',
    },
    stats: {
      total: 'Total',
      ok: 'Correctos',
      failed: 'Fallidos',
      unit: 'accesos',
    },
    result: {
      OK: 'Correcto',
      INVALID_CREDENTIALS: 'Credenciales inválidas',
      LOCKED: 'Cuenta bloqueada',
      INACTIVE: 'Cuenta inactiva',
      NO_ACCESS: 'Sin acceso',
      FALLBACK_OK: 'Correcto (contingencia)',
    },
    phase: {
      PHASE_1: 'Fase 1 (local)',
      PHASE_2: 'Fase 2 (SSO)',
      FALLBACK: 'Contingencia',
    },
    columns: {
      occurredAt: 'Fecha',
      login: 'Usuario',
      result: 'Resultado',
      phase: 'Fase',
      ip: 'IP',
    },
    empty: 'No hay accesos que mostrar.',
    loadError: 'No se pudieron cargar los accesos.',
    pagination: {
      previous: 'Anterior',
      next: 'Siguiente',
      pageInfo: 'Página {{page}} de {{total}}',
    },
    errors: {
      window: 'La fecha «Desde» debe ser anterior o igual a la fecha «Hasta».',
    },
  },
  settings: {
    navLabel: 'Configuración',
    eyebrow: 'Administración',
    title: 'Configuración del sistema',
    description: 'Ajustes globales que afectan a cómo se resuelven las solicitudes.',
    loadError: 'No se pudo cargar la configuración.',
    save: 'Guardar',
    saved: 'Configuración actualizada.',
    saveError: 'No se pudo actualizar la configuración.',
    parkingAddress: {
      heading: 'Dirección del parking',
      description: 'Se usa en el botón "Ir al parking" del empleado (abre Google Maps).',
      label: 'Dirección',
      placeholder: 'Ej.: Av. de Europa 18, 28108 Alcobendas, Madrid',
      hint: 'Déjalo en blanco para quitar el botón "Ir al parking".',
      enableLabel: 'Mostrar el botón "Ir al parking" a los empleados',
      map: {
        hint: 'Arrastra el marcador o haz clic en el mapa para fijar el punto exacto.',
        search: 'Buscar en el mapa',
        satellite: 'Satélite',
        streets: 'Mapa',
        coords: 'Punto fijado: {{lat}}, {{lng}}',
        searchError: 'No se encontró la dirección. Coloca el marcador a mano.',
        noToken: 'Mapa no disponible: falta configurar el token de Mapbox (VITE_MAPBOX_TOKEN).',
      },
    },
    weekend: {
      heading: 'Reservas en fin de semana',
      description: 'Controla si los empleados pueden reservar sábados y domingos.',
      label: 'Permitir reservar sábados y domingos',
      hint: 'Desactivado: los fines de semana no se muestran ni se pueden reservar.',
    },
    notifications: {
      heading: 'Notificaciones',
      description: 'Controla los canales por los que el sistema avisa (email y push).',
      email: 'Enviar avisos por email',
      push: 'Enviar avisos push',
      hint: 'Cada canal es independiente. Un empleado solo recibe un aviso si el canal global y el suyo están activos.',
    },
    approvalMode: {
      label: 'Modo de aprobación de solicitudes',
      heading: 'Resolución de solicitudes',
      description:
        'Define cómo se resuelven las nuevas solicitudes de plaza o puesto de los empleados.',
      activeBadge: 'Modo actual',
      short: {
        MANUAL: 'Manual',
        AUTOMATIC: 'Automático',
      },
      options: {
        MANUAL: 'Manual (el administrador aprueba)',
        AUTOMATIC: 'Automático (asignación inmediata)',
      },
      hints: {
        MANUAL: 'Cada solicitud queda pendiente hasta que un administrador la resuelve.',
        AUTOMATIC:
          'Las solicitudes se aprueban al instante asignando un recurso libre; si no hay disponibilidad, se rechazan.',
      },
    },
  },
  wizard: {
    title: 'Nueva reserva',
    progressLabel: 'Paso {{current}} de {{total}}',
    steps: {
      resource: 'Recurso',
      dates: 'Fechas',
      employee: 'Empleado',
      location: 'Ubicación',
      locationParking: 'Ubicación · Plaza',
      locationDesk: 'Ubicación · Puesto',
      summary: 'Resumen',
    },
    nav: {
      back: 'Atrás',
      next: 'Siguiente',
      confirm: 'Confirmar reserva',
      confirming: 'Creando…',
      done: 'Hecho',
      another: 'Otra reserva',
      goToStep: 'Ir al paso {{label}}',
    },
    resource: {
      lead: '¿Qué quieres reservar? Puedes elegir plaza, puesto o ambos.',
      parking: 'Plaza de parking',
      parkingDesc: 'Una plaza para el vehículo del empleado.',
      parkingTag: 'Movilidad',
      desk: 'Puesto de oficina',
      deskDesc: 'Un puesto de trabajo señalado en el plano de la planta.',
      deskTag: 'Espacio de trabajo',
    },
    dates: {
      modeLabel: 'Modo de selección de fechas',
      mode: {
        SINGLE: 'Día concreto',
        RANGE: 'Rango',
        SCATTER: 'Días sueltos',
      },
      hint: {
        SINGLE: 'Elige un único día en el calendario.',
        RANGE: 'Elige la fecha de inicio y la de fin; se incluirán todos los días intermedios.',
        SCATTER: 'Pincha varias fechas sueltas; vuelve a pincharlas para quitarlas.',
      },
      prevMonth: 'Mes anterior',
      nextMonth: 'Mes siguiente',
      selectedCount_one: '{{count}} fecha seleccionada',
      selectedCount_other: '{{count}} fechas seleccionadas',
      none: 'Aún no has elegido ninguna fecha.',
      remove: 'Quitar {{date}}',
    },
    employee: {
      lead: 'Elige el empleado para quien creas la reserva.',
      searchLabel: 'Buscar empleado',
      searchPlaceholder: 'Buscar por nombre…',
      listLabel: 'Empleados',
      noMatch: 'Ningún empleado coincide con la búsqueda.',
      error: 'No se han podido cargar los empleados.',
    },
    beneficiary: {
      lead: 'Elige para quién es la reserva: un empleado o un visitante.',
      typeLabel: 'Tipo de beneficiario',
      employee: 'Empleado',
      visitor: 'Visitante',
      searchVisitor: 'Buscar por nombre o documento…',
      noVisitor: 'Ningún visitante coincide con la búsqueda.',
    },
    location: {
      leadDesk: 'Elige un puesto que esté libre en todas las fechas seleccionadas.',
      leadParking:
        'Elige una plaza libre en todas las fechas, o deja que se asigne automáticamente por categoría.',
      checking: 'Comprobando disponibilidad…',
      error: 'No se ha podido comprobar la disponibilidad.',
      eligibleCount_one: '{{count}} recurso disponible en todas las fechas',
      eligibleCount_other: '{{count}} recursos disponibles en todas las fechas',
      anyFree: 'Asignación automática',
      anyFreeDesc:
        'Se asignará la plaza según la categoría del empleado (los cargos superiores, plantas altas). Verás la plaza exacta en el resumen antes de confirmar.',
      orPick: 'o elige una plaza concreta',
      suggestedForCategory: 'Sugeridas para {{category}} · planta {{floor}}',
      otherSpaces: 'Otras plazas',
      mapHint: 'Arrastra un recuadro para acercar · minimapa para moverte · Esc vuelve',
      noneParking: 'No hay ninguna plaza libre en todas las fechas elegidas.',
      noneDesk: 'No hay ningún puesto libre en todas las fechas elegidas.',
    },
    summary: {
      resourceType: 'Tipo de recurso',
      employee: 'Empleado',
      visitor: 'Visitante',
      category: 'Categoría',
      location: 'Ubicación',
      locationOf: 'Ubicación · {{resource}}',
      dates_one: '{{count}} fecha',
      dates_other: '{{count}} fechas',
      emailNotice:
        'Al confirmar, se notificará por email a {{name}} y su reserva quedará confirmada.',
      noEmailNotice:
        'Reserva de visitante: no se envía ningún email. La reserva quedará confirmada.',
      unknownEmployee: 'Empleado',
      autoAssignTitle: 'Plaza asignada automáticamente',
      autoAssignHint: 'Según la categoría del empleado, por fecha:',
      autoResolving: 'Calculando la plaza…',
      autoError: 'No se ha podido calcular la plaza.',
      autoSpace: 'Plaza {{number}} · Planta {{floor}}',
      autoNoSpace: 'Sin plaza libre esa fecha',
      perDayTitle: 'Asignación por día',
    },
    perday: {
      modeLabel: 'Modo de asignación',
      modeAll: 'Misma para todos',
      modePerDay: 'Distinta por día',
      listLabel: 'Días seleccionados',
      unset: 'Sin asignar',
      auto: 'Automática',
      applyToAll: 'Aplicar a todos los días',
      autoEmpties: 'Auto en los días vacíos',
      editing: 'Editando: {{date}}',
    },
    rail: {
      title: 'Tu reserva',
      label: 'Pasos y resumen de la reserva',
    },
    result: {
      successTitle: '¡Reservas creadas!',
      mixedTitle: 'Reservas creadas con incidencias',
      summary: '{{created}} creadas · {{failed}} no disponibles · notificado a {{name}}',
      summaryVisitor: '{{created}} creadas · {{failed}} no disponibles · visitante {{name}} (sin email)',
      reasonOk: 'Creada',
      reasonNoAvailability: 'Sin disponibilidad',
      reasonDuplicate: 'Ya reservada',
      reasonGeneric: 'No se pudo crear',
    },
  },
  vehicles: {
    status: {
      PENDING: 'Pendiente',
      IN_PROGRESS: 'En trámite',
      APPROVED: 'Aprobado',
      REJECTED: 'Rechazado',
      PENDING_DELETION: 'Pendiente de borrado',
    },
    review: {
      navLabel: 'Vehículos',
      eyebrow: 'Operativa',
      title: 'Validación de vehículos',
      pendingBadge: '{{count}} vehículos pendientes',
      loadError: 'No se pudieron cargar los vehículos.',
      empty: 'No hay vehículos que mostrar.',
      search: {
        label: 'Buscar vehículo',
        placeholder: 'Buscar empleado o matrícula…',
      },
      tabs: {
        ABIERTOS: 'Abiertos',
        PENDING: 'Pendientes',
        IN_PROGRESS: 'En trámite',
        APPROVED: 'Aprobados',
        REJECTED: 'Rechazados',
        PENDING_DELETION: 'Pendientes de borrado',
        ALL: 'Todos',
      },
      columns: {
        employee: 'Empleado',
        vehicle: 'Vehículo',
        plate: 'Matrícula',
        status: 'Estado',
        actions: 'Acciones',
      },
      actions: {
        inProgress: 'En trámite',
        approve: 'Aprobar',
        reject: 'Rechazar',
        restore: 'Restaurar',
        confirmDelete: 'Confirmar borrado',
        changeStatus: 'Cambiar estado',
        history: 'Histórico',
      },
      toasts: {
        inProgress: 'Vehículo marcado en trámite.',
        approved: 'Vehículo aprobado.',
        rejected: 'Vehículo rechazado.',
        restored: 'Vehículo restaurado.',
        deleted: 'Vehículo borrado.',
        statusChanged: 'Estado actualizado.',
      },
      changeStatus: {
        title: 'Cambiar estado de {{plate}}',
        label: 'Nuevo estado',
        confirm: 'Guardar',
        reasonLabel: 'Motivo del rechazo',
        reasonRequired: 'El motivo es obligatorio.',
        approveNote:
          'Al aprobar confirmas que el coche ya puede acceder al aparcamiento. Si aún no se ha contactado con la mutua, elige «En trámite».',
      },
      errors: {
        conflict: 'El vehículo ya fue procesado por otra persona.',
        generic: 'No se pudo completar la acción.',
      },
      reject: {
        title: 'Rechazar vehículo',
        confirm: 'Rechazar',
        context: 'Vas a rechazar el vehículo {{plate}}. Indica el motivo; se enviará al empleado.',
        reasonLabel: 'Motivo del rechazo',
        reasonRequired: 'El motivo es obligatorio.',
        emailNotice: 'Se notificará al empleado por email y push.',
      },
      confirmApprove: {
        title: 'Aprobar vehículo',
        body: '¿Confirmas que el coche {{plate}} de {{name}} ya puede acceder al aparcamiento? Si aún no se ha contactado con la mutua, márcalo como «En trámite» en su lugar.',
        confirm: 'Sí, aprobar',
      },
      confirmDelete: {
        title: 'Confirmar borrado',
        body: '¿Confirmas el borrado del vehículo {{plate}} de {{name}}? Esta acción no se puede deshacer.',
        confirm: 'Sí, borrar',
      },
      history: {
        title: 'Histórico de {{plate}}',
        loadError: 'No se pudo cargar el histórico.',
        empty: 'Sin cambios registrados.',
        previousData: 'Datos previos: {{plate}} · {{brand}} · {{model}} · {{color}}',
        statusChanged: 'Cambió a {{status}}',
        event: {
          CREATED: 'Alta del vehículo',
          EDITED: 'Datos modificados por el empleado',
          STATUS_CHANGED: 'Cambio de estado',
          DELETION_REQUESTED: 'Solicitud de borrado',
        },
        actor: {
          EMPLOYEE: 'Empleado',
          ADMIN: 'Administrador',
        },
      },
    },
    mine: {
      navLabel: 'Mis vehículos',
      eyebrow: 'Mi espacio',
      title: 'Mis vehículos',
      description:
        'Registra los vehículos con los que vendrás a la oficina. Cada alta o cambio queda pendiente de validación por un administrador.',
      formNotice:
        'Al guardar, el vehículo quedará pendiente de validación por un administrador. Podrás usarlo cuando lo aprueben.',
      savedToast: 'Vehículo enviado. Pendiente de validación.',
      rejectedReason: 'Rechazado: {{reason}}',
      empty: 'Aún no has añadido ningún vehículo.',
      loadError: 'No se pudieron cargar tus vehículos.',
      add: 'Añadir vehículo',
      edit: 'Editar',
      editTitle: 'Editar vehículo',
      delete: 'Borrar',
      confirmDelete: 'Sí, borrar',
      deleteTitle: 'Borrar vehículo',
      deleteBody: '¿Seguro que quieres borrar el vehículo {{plate}}? Esta acción no se puede deshacer.',
      saveVehicle: 'Guardar vehículo',
      plateRequired: 'La matrícula es obligatoria.',
      plateTaken: 'Ya tienes un vehículo con esa matrícula.',
      saveError: 'No se pudo guardar el vehículo.',
      form: {
        plate: 'Matrícula *',
        brand: 'Marca',
        model: 'Modelo',
        color: 'Color',
      },
    },
  },
  errors: {
    forbidden: 'No tienes permisos para esta operación',
    server: 'Ha ocurrido un error. Inténtalo de nuevo.',
  },
};
